...
let bufferSize = MemoryLayout<Float>.stride * vertices.count
let simdBufferSize = MemoryLayout<SIMD3<Float>>.stride * vertSimd.count
let simdElementSize = MemoryLayout<SIMD3<Float>>.size

guard let vertexBuffer = device.makeBuffer(bytes: vertSimd,
                                     length: simdBufferSize,
                                     options: []) else {
...
vertex float4 vertex_main(constant float3 *pos,
